...
const ClaimCoupon = () => {
  ...
  const setCookie = (name, value, hours) => {
    const date = new Date();
    date.setTime(date.getTime() + hours * 60 * 60 * 1000); // Cookie expiration time
  ...
  const checkRemainingTime = () => {
    const claimTime = getCookie('couponClaimed'); // Get the timestamp from the cookie
    if (claimTime) {
      const now = Date.now();
      const elapsedTime = Math.floor((now - claimTime) / 1000); // Time passed since claim in seconds
      const cooldownTime = 3600; // Cooldown period in seconds (1 hour)
      const remainingTime = Math.max(0, cooldownTime - elapsedTime);
      setRemainingTime(remainingTime);
    } else {
      setRemainingTime(0); // No restriction if no cookie exists
    }
  };
  ...
  const handleSpinClick = async () => {
    ...
    try {
      const response = await fetch(`${port}claim-coupon`, {
        method: 'POST',
      ...
      if (!response.ok) {
        ...
        if (match) {
          ...
          if (index !== -1) {
            ...
            // Set a cookie with the current timestamp after claiming
            setCookie('couponClaimed', Date.now(), 1); // Valid for 1 hour
          } else {
...
//     try {
//       const response = await fetch(`${port}check-time`);
//       const data = await response.json();
